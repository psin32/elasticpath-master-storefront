...
export const fetchSimilarProducts = async (id: string) => {
  const res = await KlevuFetch(similarProducts([id]));
...
export const fetchProducts = async (
  searchSettings: Partial<KlevuSearchOptions>,
  query?: string,
  manager?: FilterManager,
) => {
  const selectedCategories =
    manager && manager.filters[0]
      ? (manager.filters[0] as KlevuFilterResultOptions).options
  ...
  const res = await KlevuFetch(
    search(
      ...
      // @ts-ignore
      manager && selectedCategories.length
        ? advancedFiltering([
            {
              key: "category",
              singleSelect: true,
...
export const fetchRecentlyViewed = async () => {
  const res = await KlevuFetch(recentlyViewed());

  return res.apiResponse.queryResults;
};

export const fetchNewArrivals = async () => {
  const res = await KlevuFetch(newArrivals());

  return res.apiResponse.queryResults;
};

export const fetchKMCRecommendations = async (id: string) => {
  const res = await KlevuFetch(kmcRecommendation(id));
