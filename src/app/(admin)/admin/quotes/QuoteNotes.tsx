...
export function QuoteNotes({
  ...
  const handleSubmitNote = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createNoteForCart({
        cart_id: cartId,
        note: newNote.trim(),
        added_by: adminName,
      });
  ...
  return (
    <div className="w-full bg-white/90 rounded-2xl shadow-lg p-6 border border-gray-100 mt-10">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <ChatBubbleLeftRightIcon className="h-6 w-6 text-brand-primary" />
          <h2 className="text-2xl font-bold text-gray-900">Quote Notes</h2>
          {notes.length > 0 && (
