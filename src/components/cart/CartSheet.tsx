    ...
    <Sheet>
      ...
      <SheetContent className="bg-white p-0 flex flex-col w-full">
        ...
        {items && items.length > 0 ? (
          <>
            {/* Items */}
            <div className="grid gap-3 p-4 flex-1 overflow-auto">
              <ul
                role="list"
                ...
                {!enableClickAndCollect &&
                  (() => {
                    ...
                    const hasMultipleLocations =
                      Object.keys(itemsByLocation).length > 1;
                    const hasAnyLocation = Object.keys(itemsByLocation).some(
                      (key) => key !== "no-location",
                    );

                    return Object.entries(itemsByLocation).map(
                      ([locationSlug, group]: [string, any]) => (
                        <Fragment key={locationSlug}>
                          {hasAnyLocation && hasMultipleLocations && (
                            <div className="w-full">
                              <div className="flex items-center gap-2 bg-brand-primary/10 border-l-4 border-brand-primary px-3 py-2 rounded-r-md">
                                <svg
