...
import {
  generateAlgoliaCSV,
  generateAlgoliaPDFHTML,
  downloadAlgoliaCSV,
  downloadAlgoliaPDF,
  extractInventoryFromHit,
  AlgoliaProductData,
  AlgoliaLocationData,
} from "../../lib/algolia-export-utils";

interface AlgoliaExportButtonProps {
  hits: SearchHit[];
  products: ProductResponse[];
  locations: AlgoliaLocationData[];
}

export default function AlgoliaExportButton({
  hits,
  products,
  locations,
}: AlgoliaExportButtonProps) {
  const handleExportCSV = () => {
    const productData: AlgoliaProductData[] = hits.map((hit) => {
      const product = products.find((p) => p.id === hit.objectID);
      if (!product) {
        ...
        return {
          product: {
            ...
            meta: {
              display_price: {
                without_tax: { formatted: "N/A" },
                with_tax: { formatted: "N/A" },
              },
            },
          } as ProductResponse,
          hit,
          inventories: extractInventoryFromHit(hit),
        };
      }
    ...
    const csvContent = generateAlgoliaCSV(productData, locations);
    const timestamp = new Date().toISOString().split("T")[0];
    downloadAlgoliaCSV(csvContent, `algolia-products-${timestamp}.csv`);
  };
  ...
  const handleExportPDF = () => {
    ...
    const htmlContent = generateAlgoliaPDFHTML(productData, locations);
    downloadAlgoliaPDF(htmlContent);
  };
