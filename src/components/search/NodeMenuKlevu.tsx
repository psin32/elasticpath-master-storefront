...
import { useFacetClicked, usePageContext } from "./ProductsProviderKlevu";
...
export default function NodeMenuKlevu(): JSX.Element {
  const pageContext = usePageContext();
  const filters = pageContext?.filters || [];
  if (!filters.length) {
    return <></>;
  }
  let categoryFilter = filters
    ? filters.find((filter) => filter.key === "category")
    : [];
