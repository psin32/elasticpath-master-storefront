...
import { DEFAULT_LIMIT } from "./PaginationKlevu";
import {
  DEFAULT_MAX_VAL,
  DEFAULT_MIN_VAL,
} from "./price-range-slider/PriceRangeSliderKlevu";
...
type ProductsSettings = {
  ...
  offset: number;
  sortBy:
    | KlevuSearchSorting.PriceAsc
    | KlevuSearchSorting.PriceDesc
    | undefined;
};
...
function searchSettings(
  ...
  if (
    productsSettings.priceRange[0] !== DEFAULT_MIN_VAL ||
    productsSettings.priceRange[1] !== DEFAULT_MAX_VAL
  ) {
...
export const ProductsProvider = ({ children }: ProductsProviderProps) => {
  ...
  const fetchProductsData = async () => {
    try {
      ...
      const priceRangeFilter: any = data
        .queriesById("search")
        .filters?.find((filter) => filter.key === "klevu_price");
      if (!initRange) {
  ...
  useEffect(() => {
    if (manager && manager.filters?.length) {
      ...
      filter.options.forEach((option) =>
        manager.deselectOption("category", option.value),
      );
    }
  ...
  return (
    <ProductsProviderContext.Provider
      value={{ page, settings, adjustSettings, facetClicked }}
    >
      {children}
    </ProductsProviderContext.Provider>
  );
